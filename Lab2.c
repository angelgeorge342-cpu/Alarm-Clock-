...
volatile RTC_t         g_Time        = {11, 40, 0, 4, 22, 2026};
...
volatile uint8_t       g_SetCol  = 0;   // 0=hour, 1=min, 2=AM/PM
...
void ClockTask(void) {
    ...
    while (1) {
        ...
        g_DisplayDirty = 1; // refresh display every second
    }
}
...
void DisplayTask(void) {
    ...
    while (1) {
        ...
        switch (g_Screen) {
            ...
            //  Row 0:  HH:MM AM/PM
            //  Row 1:  M/DD/YY
            //  Row 2:  [>] Alarm  HH:MM  (highlighted if cursor=1)
            //  Row 4:     :SS     (live seconds)
            // ------------------------------------------------
            case SCREEN_HOME: {
                ...
                snprintf(buf, sizeof(buf), "%2u:%02u %s",
                         ...
                         isPM ? "PM" : "AM");
                BSP_LCD_DrawString(0, 0, buf, LCD_WHITE);

                snprintf(buf, sizeof(buf), "%u/%u/%02u",
                         (unsigned)mo, (unsigned)dy,
                         (unsigned)(yr % 100u));
                BSP_LCD_DrawString(0, 1, buf, LCD_WHITE);

                // Alarm row — highlight when cursor==1
                uint16_t aColor = (g_Cursor == 1) ? LCD_YELLOW : LCD_CYAN;
                if (g_Cursor == 1) {
                    BSP_LCD_DrawString(0, 2, ">", LCD_YELLOW);
                } else {
                    BSP_LCD_DrawString(0, 2, " ", LCD_BLACK);
                }
                if (g_AlarmEnabled) {
                ...
                BSP_LCD_DrawString(1, 2, buf, aColor);

                // Live seconds indicator
                snprintf(buf, sizeof(buf), "   :%02u", (unsigned)sc);
                BSP_LCD_DrawString(0, 4, buf, LCD_GREEN);
                break;
            }
...
void JoystickTask(void) {
    ...
    while (1) {
        ...
        if (g_Screen == SCREEN_HOME) {
            ...
            if (canAct && jy < JOY_LO) {          // DOWN ? select Alarm
                g_Cursor = 1;
                g_DisplayDirty = 1;
                lastAct = now;
            }
            if (canAct && jy > JOY_HI) {          // UP ? deselect
                g_Cursor = 0;
                g_DisplayDirty = 1;
                lastAct = now;
            }
            // Joystick click (falling edge) ? enter alarm set if highlighted
            if ((sel == 0) && (prevSel != 0) && canAct) {
                lastAct = now;
                if (g_Cursor == 1) {
                    // Pre-load UI from current saved alarm
                    if (g_AlarmEnabled) {
...
void AlarmBuzzerTask(void) {
    ...
    while (1) {
        ...
        while (!handled) {
            ...
            // g_SemaStop is a plain int32_t; a value >0 means signaled.
            if (g_SemaStop > 0) {
...
int main(void) {
    ...
    // Button2: PD7 (used as "Done" in alarm-set screen)
    BSP_Button2_Init();
